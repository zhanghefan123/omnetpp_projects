...
class INET_API NetworkInterface : public cSimpleModule, public queueing::IPassivePacketSink, public queueing::IPacketProcessor, public ILifecycle, public cListener
{
  ...
  private:
    simtime_t deleteInterval = 10;
    std::map<uint64_t, simtime_t> nonceMap;
  public:
    // zhf add code for lipsin loop prevention
    void setDeleteInterval(simtime_t deleteIntervalTmp){deleteInterval = deleteIntervalTmp;};
    void insertNonce(uint64_t n){this->nonceMap[n] = simTime();};
    bool findNonce(uint64_t n);
