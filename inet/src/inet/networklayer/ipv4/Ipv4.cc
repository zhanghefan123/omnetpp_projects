...
void Ipv4::preroutingFinish(Packet *packet)
{
    ...
    // if the destination address is the multicast address
...
void Ipv4::forwardMulticastPacket(Packet *packet)
{
    ...
    // find the bestMatching route with the help of source address and destination address
    ...
    if (!route) {
        ...
        // zhf add code: emit signals to notify the pim-dm to create route
    ...
    // if the pacekt not arrived on the input interface
    ...
    // after the checking we can forward the packet out
    else {
        // TODO no need to emit fromIE when tags will be used in place of control infos
        ...
        for (unsigned int i = 0; i < route->getNumOutInterfaces(); i++) {
            ...
            // the destIE is the output interface
            // the fromIE is the input interface
            if (destIE != fromIE && outInterface->isEnabled()) {
                ...
                // if outputInterface is leaf and no further listeners
                ...
                // forward the packet
                    ...
                    // duplicate the packet and forward it out
                    auto packetCopy = packet->dup();
