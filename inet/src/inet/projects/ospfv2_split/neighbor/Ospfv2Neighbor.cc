...
{
    // ============= zhf add code to judge wheather go to two way =============
    bool needAdjacencyFromAreaPerspective = true;
    int neighbor_area = this->getArea();
    int current_node_area = int(this->getInterface()->getArea()->getRouter()->get_containing_module()->getParentModule()->par("area").intValue());
    if(neighbor_area != current_node_area) {
        needAdjacencyFromAreaPerspective = false;
    }
    // ============= zhf add code to judge wheather go to two way =============
    if(needAdjacencyFromAreaPerspective){
