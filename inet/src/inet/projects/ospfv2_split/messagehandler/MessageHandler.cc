...
void MessageHandler::sendPacket(Packet *packet, Ipv4Address destination, Ospfv2Interface *outputIf, short ttl)
{
    ...
    // zhf add code
    // --------------------------------------------------------------
    GlobalModule* global_module = dynamic_cast<GlobalModule*>(this->ospfModule->getSystemModule()->getSubmodule("globalModule"));
    if(omnetpp::simTime().dbl() > global_module->get_start_to_fail()){
        std::ofstream output_file;
        output_file.open("send_packet_recording.txt", std::ios::app);
        if(ospfPacket->getType() != HELLO_PACKET){
            global_module->update_ospf_consumption_in_bytes(packet->getByteLength());
            std::string module_full_path = this->ospfModule->getParentModule()->getFullPath();
            if(ospfPacket->getType() == DATABASE_DESCRIPTION_PACKET) {
                output_file << module_full_path << " send DD packet " << packet->getByteLength() << std::endl;
                global_module->ddCount += 1;
            }
            else if(ospfPacket->getType() == LINKSTATE_REQUEST_PACKET) {
                output_file << module_full_path << " send LSR packet " << packet->getByteLength() << std::endl;
                global_module->lsrCount += 1;
            }
            else if(ospfPacket->getType() == LINKSTATE_UPDATE_PACKET)
            {

                output_file << module_full_path << " send LSU packet " << packet->getByteLength() << std::endl;
                global_module->lsuCount += 1;
            }
            else if(ospfPacket->getType() == LINKSTATE_ACKNOWLEDGEMENT_PACKET)
            {
                output_file << module_full_path << " send LSAck packet " << packet->getByteLength() << std::endl;
                global_module->lsaAck += 1;
            }
        }
        output_file.close();
    }
    ...
    // zhf add code -- record the control cost
    // ------------------------------------------------------------------------
//    std::string global_module_name = "globalModule";
//    auto* global_module = dynamic_cast<GlobalModule*>(this->ospfModule->getSystemModule()->getSubmodule(global_module_name.c_str()));
//    if(global_module){
//        global_module->record_ospf_control_packet_size(int(packet->getByteLength()));
//    }
    // ------------------------------------------------------------------------
