...
#include "inet/projects/ospfv2_split/router/Lsa.h"
...
{
    this->containing_module = containingModule;  // zhf add code
...
void Router::rebuildRoutingTable()
{
    ...
    if (!diffEraseEntries.empty() || !diffAddEntries.empty()) {
        ...
        for (auto& entry : diffAddEntries)
            EV_INFO << "added: " << entry << "\n";
        // zhf add code: the routing table changed: we need to record in the global module
        // -------------------------------------------------------------------------------
        if(this->containing_module){
            std::string global_module_name = "globalModule";
            auto* global_module = dynamic_cast<GlobalModule*>(this->containing_module->getSystemModule()->getSubmodule(global_module_name.c_str()));
            // std::cout << "routing table update time: " << omnetpp::simTime().dbl() << std::endl;
            if(global_module){
                global_module->update_routing_table_rebuild_time();
            }
        }
        // -------------------------------------------------------------------------------
    }
...
// zhf add code
cModule* Router::get_containing_module(){
    return this->containing_module;
}
