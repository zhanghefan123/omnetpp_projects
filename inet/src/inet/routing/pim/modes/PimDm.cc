 ...
 *
 * zhf add annotation: the router first received packet from (source, group)
 */
void PimDm::unroutableMulticastPacketArrived(Ipv4Address source, Ipv4Address group, unsigned short ttl)
{
    ...
    // the group is multicast
    ASSERT(group.isMulticast());

    EV_DETAIL << "New multicast source observed: source=" << source << ", group=" << group << ".\n";

    // we need to find best matching route to source (RPF - reverse path forwarding)
    Ipv4Route *routeToSrc = rt->findBestMatchingRoute(source);

    // if we cannot find the route to source <-- may be because we have no ospf routing protocol to construct global routing table
    if (!routeToSrc || !routeToSrc->getInterface()) {
        std::cout << "cannot find route to source" << std::endl;
        EV << "ERROR: PimDm::newMulticast(): cannot find RPF interface, routing information is missing.";
        return;
    }

    // get the route to source and fetch the output interface along the path
    PimInterface *rpfInterface = pimIft->getInterfaceById(routeToSrc->getInterface()->getInterfaceId());

    // if we cannot find the rpfInterface or the interface is not DenseMode return
    if (!rpfInterface || rpfInterface->getMode() != PimInterface::DenseMode)
        return;

    // we need to judge if the source is directly connect to the router
    ...
    // create new route - this route is a pim-dm route
    Route *route = new Route(this, source, group);
    // routes is a mapping from {(source,group)--->route}
    routes[SourceAndGroup(source, group)] = route;
    // set up the route metric
    ...
    if (isSourceDirectlyConnected) {
        // set up the originator state {NOT_ORIGINATOR, ORIGINATOR}
    ...
    // is all downstream interfaces are pruned
    bool allDownstreamInterfacesArePruned = true;

    // insert all PIM interfaces except rpf interface
    for (int i = 0; i < pimIft->getNumInterfaces(); i++) {
        ...
        // check if has multicast listeners
        bool hasConnectedReceivers = pimInterface->getInterfacePtr()->getProtocolData<Ipv4InterfaceData>()->hasMulticastListener(group);

        // if has pim router neighbors or has directly connected receivers | we will forward the packet
    ...
    // create new multicast route and insert it into the routing table
