...
void terNetMobility::initialize(int stage)
{
    switch (stage) {
    case 0: {
 ...
 */
Coord terNetMobility:: getTransformPosition()
{
    // reading the coordinates from omnetpp.ini makes predefined scenarios a lot easier
       //auto coordinateSystem = getModuleFromPar<OsgGeographicCoordinateSystem>(par("coordinateSystemModule"), this);
       auto coordinateSystem = getModuleFromPar<IGeographicCoordinateSystem>(par("coordinateSystemModule"), this);
       if (coordinateSystem != nullptr && hasPar("latitude") && hasPar("longitude") && hasPar("altitude")) {
           auto initialLatitude = deg(par("latitude"));
           auto initialLongitude = deg(par("longitude"));
           auto initialAltitude = m(par("altitude"));
           position = coordinateSystem->computeSceneCoordinate(GeoCoord(initialLatitude, initialLongitude, initialAltitude));
       }
       return position;
}

void terNetMobility::refreshVisuals()
{
    geoTransform->setPosition(osgEarth::GeoPoint(mapNode->getMapSRS(), longitude, latitude, altitude));
    lastPosition= getTransformPosition();
}

const Coord& terNetMobility::getCurrentPosition()
{
    return lastPosition;
}
//void terNetMobility::handleMessage(cMessage *msg)
//{
